...
#ifndef BROWSER_PROVIDER_LOG_H
#define BROWSER_PROVIDER_LOG_H

#ifdef SUPPORT_LOG_MESSAGE

#ifdef SUPPORT_FILE_LOGGING
#include <errno.h>
#include <stdio.h>
#include <stdarg.h>
...
#define TRACE_SECURE_INFO(format, ARG...) bp_ftrace("[%s][%s:%d] "format"\n", LOG_TAG, __FUNCTION__, __LINE__, ##ARG)
#define TRACE_SECURE_ERROR(format, ARG...) bp_ftrace("[%s][%s:%d] "format"\n", LOG_TAG, __FUNCTION__, __LINE__, ##ARG)

#else // No FILE Logging, DLOG message
